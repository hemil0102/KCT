...
## 리팩토링에서 나온 질문들 (2026-09-23)

### Q. `answerPool` 이 뭐 하는 거지? (`QuestionCatalog.swift` 의 ❓)

**선다형의 「오답 보기」를 뽑아 오는 통**입니다.

문항마다 오답을 손으로 적어 두는 대신, **다른 문제들의 정답**을 모아 두고 거기서 뽑습니다.

- 오답이 그럴듯합니다 — 「고조선」의 오답으로 「고구려」가 나옵니다. 아무 말이나 넣으면 눌러 보지 않아도 답이 보입니다
- 문제집을 갈아끼우면 **보기도 저절로 따라 바뀝니다** — 1,130문항으로 늘려도 오답을 다시 적을 일이 없습니다

같은 보기가 두 번 나오면 안 되므로 `Set` 을 거쳐 중복을 없앱니다.

다만 무작위로 뽑으면 같은 문항의 난이도가 회차마다 달라지므로, 두 가지로 좁혀 씁니다.

| 함수 | 언제 | 무엇 |
|---|---|---|
| `answerPool(excludingCategory:)` | 격려용(첫·마지막 칸) | 같은 계열을 **빼서** 일부러 쉽게 |
| `answerPool(preferringKindOf:count:)` | 나머지 | 같은 종류(`AnswerKind`)로 최대한 채우고 **모자란 자리만** 다른 데서 |

> 뒤엣것에 `count` 를 받는 이유 — 2지선다는 오답 1개, 4지선다는 3개가 필요합니다.
> 필요한 만큼만 요구해야 「인물이 2명뿐이라 아쉽지만 못 쓴다」는 일이 안 생깁니다.

### Q. `ContentFile` 은 왜 받은 JSON 을 다시 `encode` 하나? (`ContentFile.swift` 의 ❓)

`saveDownloaded(_:)` 가 `Payload` 를 받아 다시 `JSONEncoder` 로 인코딩하는 것이 이상해 보입니다.
받은 것이 이미 JSON 이니 그 바이트를 그대로 쓰면 될 텐데요.

**맞습니다. 지금 서명이 잘못 잡혀 있습니다.**

- **좋은 점** — 타입이 한 번 걸러집니다. 서버가 깨진 JSON 을 줬으면 **저장하기 전에** 알 수 있습니다
- **나쁜 점** — `Data → Payload → Data` 를 두 번 거치므로 서버가 준 것과 저장된 것이 **글자 단위로 다릅니다**(키 순서·공백). 그리고 `Payload` 에 없는 키는 **조용히 사라집니다** — 나중에 앱을 업데이트하면 그 키를 읽고 싶어도 이미 없습니다

⚠️ **아직 부르는 곳이 없어 실제 문제가 된 적은 없습니다.** 서버 갱신을 실제로 붙일 때
`saveDownloaded(_ data: Data)` 로 바꾸고 **검증은 따로**(받은 `Data` 를 한 번 디코드해 보고,
저장은 원본 바이트로) 하는 것이 맞습니다. `Brainstorm.md` 에 적어 두었습니다.

### Q. 중복을 없애다가 기능이 사라질 수 있나?

**있습니다. 이번에 실제로 겪었습니다.**

네 곳의 모델 호출이 같은 넷(세션 만들기 → 물어보기 → `catch` → 실패 기록)을 되풀이하고
있어 `ModelCall.generate` 하나로 모았습니다. 그런데 낱말 예문에는 한 가지가 더 있었습니다 —
세이프티 가드레일에 걸렸을 때 애플에 신고할 첨부자료를 만드는
`session.logFeedbackAttachment(...)`.

그 API 는 **그 세션 객체가 있어야** 부를 수 있습니다. 호출을 공통 함수로 감싸면
`catch` 가 함수 안에 들어가므로, **밖에서는 세션에 손이 닿지 않습니다.**

```swift
// 합치기 전 — catch 안에서 session 이 보인다
let session = LanguageModelSession(instructions: instructions)
do { ... } catch {
    let attachment = session.logFeedbackAttachment(...)   // ← 여기가 사라진다
}
```

해결은 **훅**입니다. `generate` 가 실패할 때 그 세션과 에러를 그대로 넘겨 주는 클로저
(`onFailure`)를 두고, 낱말 예문만 그 자리에서 첨부자료를 만듭니다.

**교훈 둘**

- **합치기 전에 「이 중 한 곳만 하는 일」이 있는지 먼저 센다.** 네 곳이 「거의」 같으면
  그 「거의」가 어디인지가 설계를 정합니다
- **빌드가 되는 것으로는 안 잡힌다.** `logFeedbackAttachment` 를 지워도 컴파일은 됩니다.
  `#if DEBUG` 안이라 릴리스에서는 경고도 안 납니다 — **기능이 있었다는 사실을 아는 사람만**
  잡을 수 있었습니다

### Q. 「똑같다」고 적힌 주석은 왜 위험한가?

`CorrectAnswerSheet.swift` 에 이런 주석이 있었습니다.

```swift
/// 이모지와 낱말 배지가 한 줄, 그 아래 왼쪽 끝부터 설명. FeedbackSheet.noteRow(...) 와 똑같다.
```

**똑같다는 것을 주석으로 적어야 하는 상태가 곧 갈라질 상태입니다.**

- 한쪽을 고치는 사람은 **다른 쪽이 있다는 것을 주석으로만** 압니다
- 주석은 컴파일되지 않으므로, 한쪽만 고쳐도 아무 일이 안 일어납니다
- 그다음부터 그 주석은 **거짓**입니다. 그런데 읽는 사람은 여전히 참이라고 믿습니다

같은 코드를 쓰게 하면 이 위험이 사라집니다 — 배지 색을 바꾸려면 `WordBadge` 를 고쳐야 하고,
고치면 **두 창이 함께** 바뀝니다. 「같아야 한다」를 약속이 아니라 **구조**로 만드는 것입니다.

> 이 저장소에서 **중복을 찾는 가장 빠른 방법**이 여기 있습니다 — 「똑같다」·「같은 모양」·
> 「같은 값」이라고 적힌 주석을 `grep` 하면 됩니다. 그 문장이 있는 자리가 곧 뺄 자리입니다.
