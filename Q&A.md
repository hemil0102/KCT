...
**낱말 사전 시트에는 `.presentationBackgroundInteraction(.enabled)`(완전히 안 어둡게)를 적용했습니다.** 문제 지문을 계속 보면서 뜻만 확인하는 용도라, 뒤가 밝게 보이고 계속 탭도 되는 쪽이 이 화면의 목적에 맞습니다. 오답 해설 시트(`FeedbackSheet`)는 건드리지 않았습니다 — 그쪽은 `.interactiveDismissDisabled()`까지 걸어 "정답을 다 보기 전엔 못 넘어가게" 만든 화면이라, 뒤가 밝고 탭까지 되면 그 의도와 어긋납니다.
