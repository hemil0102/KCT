...
- **현재 단계** — **1차 리팩토링 완료.** 무엇을 왜 고쳤는지는 [Refactoring.md](Refactoring.md)
- **마지막으로 끝낸 것** — 코드 전체(47파일 8,715줄)를 훑어 ① 죽은 코드(색 9개·`question(id:)`·`ContentFile.load()`·`Question.tags`·`PrimaryActionButton.fill`) ② 중복(두 해설 시트의 공통 부품, X버튼 3곳, 안내 배너 2곳, 누적 카드 2곳, 흔들림 3중복, `PreferenceKey` 2→1, 진행 막대, 모델 호출 보일러플레이트, 업로더, 카탈로그 로딩) ③ 자리(`composeExample` 을 `Data`→`Grading`, 최상위 `Coordinator`→`KoreanText.Coordinator`, `Commentary` 타입을 `QuizSession` 밖으로, 200줄 넘는 파일 3개 쪼개기) 를 정리했습니다. **새 파일 12개 · 사라진 파일 1개 · 단계마다 빌드 12회 전부 성공.** 거짓이던 주석 13곳과 DocC 5장도 함께 고쳤습니다. `questions.json` 은 한 글자도 안 바꿨습니다(`CodingKeys` 맵핑)
- **다음 할 일** — **실기기 확인.** 리팩토링은 동작이 안 바뀌어야 성공이라 빌드만으로는 부족합니다. 확인 목록 9개는 [Refactoring.md](Refactoring.md) 맨 아래에 있습니다 — 특히 ① 낱말 사전 시트가 아래로 쓸어내려 닫히는지(`blocksInteractiveDismiss: false`) ② 지문 **마지막 줄** 점선이 여전히 보이는지(`UnderlineLabel` 이사) ③ 흔들림 세 박자(`shake` 통합) ④ `#if DEBUG` 첨부자료가 여전히 남는지(`onFailure` 훅). 그다음은 [Brainstorm.md](Brainstorm.md) 의 도입 후보 중에서 고릅니다
- **막힌 것** — 없음. `model_failure` 표와 RLS 정책(`job = 'glossary_example'` 허용)은 11차에 해결됐습니다
- **임시로 꺼 둔 것** — `KoreanText.swift` 의 형광펜(marker, 노란 배경)을 `showsMarkerHighlight = false` 로 잠시 껐습니다(밑줄 버그를 따로 보려는 진단용). 리팩토링에서도 **그대로 뒀습니다** — 되돌리려면 그 값을 `true` 로

**이번에 얻은 가장 큰 것** — **중복 제거가 기능을 지우면 리팩토링이 아닙니다.** 네 곳의 모델 호출을 `ModelCall.generate` 하나로 합쳤더니, 세이프티 가드레일 첨부자료를 만드는 `session.logFeedbackAttachment` 가 **조용히 사라졌습니다** — 그 API 는 세션 객체가 있어야 부를 수 있는데 호출을 함수로 감싸면 세션에 손이 닿지 않기 때문입니다. `onFailure` 훅으로 세션을 그대로 넘겨 살렸습니다. 그리고 **「똑같다」를 주석으로 적어야 하는 상태가 곧 갈라질 상태입니다** — `CorrectAnswerSheet` 가 「FeedbackSheet 의 것과 똑같다」고 적고 있던 곳이 정확히 부품으로 뺄 자리였습니다. ([Q&A.md](Q&A.md))
...
|---|---|:---:|
| `App/` | 앱 시작점 | 2 |
| `Data/` | 문제·연결문제 (고정) — 아무것도 모른다. `questions.json`(**30문항**, 문항마다 `glossary` 내장·39낱말) · `matchingSets.json` · `glossary.json`(예전 자료, 안 씀) | 5 |
| `Progress/` | 학습 진척 (변동) + `ModelContext.fetchKeyed` | 3 |
| `Session/` | 이번 회차 출제 구성 | 3 |
| `Grading/` | 채점 세 층 + **모델에게 글 시키기**(`ModelCall`·`Writing`·writer 셋) + 해설 창 내용 + 답을 보는 낱말들 | 10 |
| `Focus/` | 묻는 대상 하이라이트 (**2층은 꺼져 있음 — 두 파일은 안 불린다**) | 4 |
| `Screens/` | 회차 상태(`QuizSession`) + 화면 8개 + 연결 문제 판 | 12 |
| `Observation/` | 관찰 기록·업로드 (Supabase) | 4 |
| `DesignSystem/` | 색·조판·낭독 + **재사용 부품**(도메인을 모른다) — 해설 창 공통 부품·버튼·배너·카드 | 14 |
| `KCT.docc/` | 랜딩 + 개념 아티클 6개. **앱 번들에 안 실린다** | 7 |
...
- **같은 모습은 같은 코드로 강제한다** (2026-09-23 리팩토링) — 「똑같이 생겼다」를 주석으로 약속하면 한쪽만 고쳐질 때 주석이 거짓이 된다. 세 해설 창은 `CommentarySheet.swift` 의 부품을 공유한다
- **실측으로 정한 값은 리팩토링에서 절대 안 건드린다** — 0.75 진하기·150/260 시트 높이·21/28pt·`underlineGap` 기준·배너 18/17pt. 「통일하면 깔끔해 보이는」 자리도 인자로 각자 값을 넘긴다
- **`ModelCall.generate` 에 `onFailure` 훅이 있는 이유** — `session.logFeedbackAttachment` 는 세션 객체가 있어야 부를 수 있다. 훅을 없애면 세이프티 신고 기능이 조용히 사라진다
...
- **안전 필터가 해설에서 종종 난다** — `Response may contain sensitive or unsafe content`. `model_failure` 표에 지침·프롬프트가 쌓이고 있으니 **어머니 회차 뒤에 그 표를 읽어 보는 것**이 다음 확인이다
...
- **음성 입력 칸을 실기기로 봐야 한다** (2026-09-25 붙임) — ① `contextualStrings` 에 정답을 넘기는데, 비슷한 소리의 오답이 정답으로 끌려 적히는지 ② 파형 상수(`BandAnalyzer.floorDB`·`rangeDB`·`silenceDB`)가 어머니 목소리에 맞는지 ③ 1.5초 침묵(`SpeechListener.silenceLimit`)이 어르신께 짧아 말 중간에 끊기는지 ④ 음성 칸의 사다리를 `nudgeLadder`(격려 칸과 같음)로 둔 것이 맞는지 — 말로 맞힌 것은 강한 증거일 수 있다
...
- **2026-09-25** — 해설 모달·색·음성 입력. ① 해설 모달 규칙 확정 — **정답**: 접힌 채로 뜨고 「정답 해설 보기」로 펼친다 / **오답**: 펼친 채로 뜨고(해설이 바로 보임) 「접기」나 아래로 내리면 접혀 「정답 해설 보기」가 나온다(`FeedbackSheet`·`TrueFalseSheet`, `CorrectAnswerSheet` 는 그대로). ② 4지선다에서 정답을 골라도 녹색이 안 되던 버그 — 타이밍이 아니라 `multipleChoiceButton` 이 `isSelected: false` 를 박아 넘기고 있었다. 오답 소거 타이머가 다음 문제에 새던 것도 같이 막음. ③ 연결 문제에서 맞힌 짝의 카드·선을 녹색(`answerSheetAccent`)으로. ④ **회차 7칸 → 8칸, 7번째 칸에 음성 입력 문제** — 새 파일 `SpeechListener`(SFSpeechRecognizer + 대역별 FFT 파형)·`VoiceAnswerPanel`(키보드 없음, 가운데 `microphone.circle.fill`, 아래 대역 파형, 말이 끝나면 곧바로 제출). `QuizItem.isVoice`, `QuizSession.placeVoiceItem()`(가장 익숙한 문항을 7번째 칸으로), `AnswerMatcher` 의 `.voice` 경로(정답이 말 속에 있으면 `containsAnswer`, 한 글자 차이는 오타로 끊지 않음), Info.plist 권한 문구 두 개. 모두 사용자 요청으로 AI 가 직접 적용했고, **이 환경엔 Xcode 가 없어 컴파일 확인은 못 했다**
- **2026-09-23** — **1차 리팩토링.** 코드 전체를 훑어 죽은 코드·중복·자리를 정리했다(13단계, 단계마다 빌드 12회 전부 성공). 가장 큰 덩어리는 **두 해설 시트의 공통 부품** — `FeedbackSheet`(413줄)와 `CorrectAnswerSheet`(357줄)가 `noteRow`·`notesCard`·`justifiedBody`·시트 외장·상수 4개를 글자 그대로 공유하고 있었고, 그 사실이 이미 주석에 「똑같다」고 적혀 있었다. `DesignSystem/CommentarySheet.swift` 로 뽑아 244/265줄로 줄였다. **의존성 방향도 한 군데 복구** — `Data/GlossaryComposer.swift` 의 최상위 함수 `composeExample` 이 `Grading` 의 `Writing` 을 쓰고 있어 `Grading/GlossaryExampleWriter.swift` 로 옮겼다(`Data` 는 다시 아무것도 모른다). 모듈 전체에 떠 있던 최상위 `Coordinator` 클래스를 `KoreanText.Coordinator` 로 중첩하고, `Commentary` 타입 둘을 `QuizSession`(900줄) 밖으로 꺼내 두 시트가 세션을 아예 모르게 했다. 200줄 넘는 파일 셋을 쪼갰다 — `KoreanText`(613→339, `UnderlineLabel` 분리)·`GlossaryPanel`(479→328, `GlossaryExampleText` 분리)·`MatchingQuestionScreen`(505→388, `MatchBoard` 분리). 네 곳의 모델 호출을 `ModelCall.generate` 한 벌로 묶으면서 **세이프티 첨부자료 기능이 조용히 사라지는 것을 발견**해 `onFailure` 훅으로 세션을 넘겨 살렸다 — 중복 제거가 기능을 지우면 리팩토링이 아니다. `AppColor` 의 쓰는 곳 0인 색 9개, `question(id:)`·`ContentFile.load()`·`Question.tags`·`PrimaryActionButton.fill` 을 지웠고, `GlossaryEntry.examples` 를 `relatedWords` 로 고치면서 `CodingKeys` 맵핑으로 `questions.json` 은 한 글자도 안 바꿨다. 실측으로 정한 값(0.75·150/260·21/28pt·배너 18/17pt)과 꺼 둔 스위치 둘(`showsMarkerHighlight`·`usesModelAnalysis`), `ObsUploader` 재진입 가드는 **전부 그대로** — 동작이 바뀌면 리팩토링이 아니라 기능 변경이다. 거짓이던 주석 13곳과 DocC 5장(`KCT.md` Topics 재편·`FeedbackModal.md` 전면 재작성·`SessionPlanning.md` 에 「연결 문제가 낀 칸」 절 신설)도 함께 고쳤다. 이력은 [Refactoring.md](Refactoring.md), 도입 후보는 [Brainstorm.md](Brainstorm.md)
...
| **리팩토링 이력** | [Refactoring.md](Refactoring.md) |
| **다음에 도입할 후보** | [Brainstorm.md](Brainstorm.md) |
