...
/// 질문이 묻는 대상.
///
/// - Important: ``phrase`` 는 **지문에 그대로 존재하는 문자열**이어야 합니다.
///   화면에서 이 문자열을 찾아 형광펜을 칠하므로, 지문에 없는 표현이면
///   아무것도 강조되지 않습니다.
...
/// 그래서 온디바이스 모델이 없는 기기에서도, 문제집에 새 문제를 넣은 직후에도
/// **기다림 없이** 형광펜이 동작합니다.
///
/// ## 왜 결과 타입과 추출기를 나눴나
///
/// 나중에 서버가 분석값을 내려주거나 모델이 보강할 수 있습니다. 그때
/// ``QuestionFocus``(결과)는 그대로 두고 만드는 쪽만 갈아끼우면 됩니다.
enum QuestionFocusExtractor {
    ...
    /// 의문사 앞 명사로 종류를 판단한다.
    ///
    /// **구체적인 것을 먼저 둡니다.** "나라의 이름은 무엇입니까?" 에서 일반적인
    /// "이름" 보다 "나라" 를 골라야 하므로, 배열 순서가 곧 우선순위입니다.
