//
//  GlossaryExampleWriter.swift
//  KCT
//
//  역할 : 낱말 사전 시트에 보여줄 짧은 예문 하나를 온디바이스 모델로 만든다
//  요점 : "낱말(뜻)" 이 문장 안에 자연스럽게 녹아든 한 문장. 실패해도 무엇을 시켰는지 남긴다
//
//  ── 구성 ──────────────────────────────────────────────
//  GlossaryExample (@Generable)   모델이 만들어 주는 문장 하나
//  └─ sentence
//
//  GlossaryExampleWriter
//  └─ write(word:gloss:relatedWords:referenceSentence:questionID:) -> Writing
//
//  파일 안의 도우미 (DEBUG 전용)
//  └─ saveFeedbackAttachment(...)   세이프티 가드레일에 걸리면 애플 신고용 .json 을 남긴다
//
//  ── 이 파일이 Grading/ 에 있는 이유 ────────────────────
//  2026-09-23 리팩토링 전에는 Data/GlossaryComposer.swift 에 **최상위 함수**
//  composeExample(...) 로 있었다. 그런데 이 코드는 FoundationModels 를 부르고
//  Grading 의 Writing 을 돌려준다 — Data 폴더는 「아무것도 모른다」가 규칙인데
//  거기서 Grading 을 아는 셈이었다(CLAUDE.md 의 「의존성은 위에서 아래로 한 방향」 위반).
//  자리를 옮기고 이름도 CommentaryWriter · EncouragementWriter 와 같은 모양
//  (구조체 + write)으로 맞췄다.
//
//  ── 흐름 ──────────────────────────────────────────────
//  QuestionScreen 이 지문의 낱말을 탭하면
//    → 시트를 먼저 띄우고(exampleState = .loading)
//    → write(...) 호출
//    → 기기가 Apple Intelligence 를 못 쓰면 곧바로 Writing.empty
//      (보낸 게 없으니 남길 실패도 없다)
//    → 오면 .ready(문장), 못 만들면 .unavailable
//    → 실패(writing.failure)는 session.saveFailures(...) → ObsUploader 로
//      Supabase model_failure 표에 job = 'glossary_example' 로 올라간다
//
//  ── 연결 ──────────────────────────────────────────────
//  불러 쓰는 곳 : QuestionScreen (낱말 탭 → onTapWord)
//  기대는 것    : FoundationModels · ModelCall.generate · Writing
//  건드리지 않는 것 : 실패를 저장·업로드하는 일 — QuizSession·ObsUploader 의 몫.
//                    화면에 어떻게 보일지 — GlossaryPanel 의 몫
//
//  ⚠️ DEBUG 빌드에서만, 세이프티 가드레일 위반(guardrailViolation)이면 애플
//     Feedback Assistant 에 첨부할 .json 파일도 문서 폴더에 남긴다. 그 파일을
//     꺼내려면 Xcode 로 기기에 연결해야 하므로, 어머니가 쓰는 실기기(RELEASE)에는
//     안 쌓아 둔다.
//
...
///
/// ``CommentaryWriter`` 와 같은 모양입니다 — 지시문과 프롬프트를 쓰고,
/// 부르고·받고·실패를 남기는 일은 ``ModelCall/generate(job:questionID:instructions:prompt:generating:options:timeout:extract:)`` 에 맡깁니다.
struct GlossaryExampleWriter {

    /// 예문 하나를 만든다. 못 만들면 ``Writing/text`` 가 `nil`.
    ///
    /// 기기가 Apple Intelligence 를 지원하지 않으면 아무 것도 시키지 않고 곧바로
    /// ``Writing/empty`` 를 돌려줍니다 — **보낸 게 없으니 남길 실패도 없습니다.**
    /// 모델을 실제로 불렀는데 실패하면(세이프티 가드레일 위반도 여기 포함된다)
    /// 보낸 지시문 · 프롬프트를 ``ModelFailureDraft`` 에 그대로 담아 돌려줍니다.
    ///
    /// - Parameters:
    ///   - word: 지문에 나온 낱말 그대로.
    ///   - gloss: 이미 다듬어 둔 짧은 뜻.
    ///   - relatedWords: 비슷한 낱말 목록. 대부분 비어 있다.
    ///   - referenceSentence: 사람이 미리 써 둔 참고 예문. "기리다"처럼 모델이
    ///     활용형을 헷갈리는 낱말에만 채워 두었고, 대부분은 `nil` 이다.
    ///   - questionID: 이 낱말이 나온 문항의 id. 실패 기록에 붙여 어느 문항에서
    ///     어떤 낱말을 탭했을 때 걸렸는지 나중에 되짚을 수 있게 한다.
    func write(
        word: String,
        gloss: String,
        relatedWords: [String] = [],
        ...
        // 기록도 없다. 패널은 JSON 의 비슷한 낱말 목록(있으면)으로 대신 보여준다.
        guard ModelCall.isAvailable else { return .empty }
        ...
        let writing = await ModelCall.generate(
            job: "glossary_example",
            questionID: questionID,
            instructions: instructions,
            prompt: prompt,
            generating: GlossaryExample.self,
            // @Guide 에 "** 로 감싸지 말라"고 적어 뒀지만, 모델이 가끔 낱말을 마크다운
            // 볼드(**낱말**)로 감싸서 내보낸다 — 지시문은 확률을 낮출 뿐 100% 막지는
            // 못한다. 화면에는 별표가 그대로 문자로 찍히고(예: "**서기전**"), 낱말
            // 강조 구간을 찾는 로직(GlossaryExampleText 의 highlightedExample)도 "(" 바로
            // 앞 공백 없는 덩어리를 낱말로 보기 때문에 별표까지 그 덩어리에 끼어
            // 들어가 버린다. 그래서 화면에 보내기 전에 코드에서 한 번 더 확실히 걷어낸다.
            extract: { $0.sentence.replacingOccurrences(of: "**", with: "") },
            onFailure: { session, error in
                #if DEBUG
                // 세이프티 가드레일에 걸렸을 때**만** 애플에 신고할 첨부자료를 남긴다 —
                // 그 외 실패(문맥 초과·응답 형식 오류 등)는 애플의 세이프티 팀이 볼
                // 대상이 아니다. 이 분기를 하려면 **그 세션 객체**가 있어야 해서
                // ModelCall.generate 에 onFailure 훅을 뒀다.
                guard let generationError = error as? LanguageModelSession.GenerationError,
                      case .guardrailViolation(let context) = generationError else { return }
