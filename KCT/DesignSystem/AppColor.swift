...
enum AppColor {
    ...
    /// **정답**을 뜻하는 진한 녹색. **두 해설 창 모두에서 정답 낱말**에 쓴다.
    ///
    /// 창 배경이 보라로 돌아간 뒤에도 이 색은 남는다 — 낱말은 흰 카드 위에
    /// 올라가므로 배경과 부딪히지 않고, 「고르신 답(분홍) ↔ 정답(녹색)」이
    /// 하늘색일 때보다 뜻이 바로 온다.
    ///
    /// 버튼 위의 흰 글자가 6.48:1, 흰 카드 위 낱말이 6.48:1, 아래 ``answerSheetBadge``
    /// 위에서 5.29:1 로 읽힌다.
    static let answerSheetAccent = Color(red: 0.180, green: 0.420, blue: 0.122)

    /// 정답 해설 모달에서 **정답 낱말 배지의 배경**. 옅은 녹색.
    ///
    /// 글자는 ``answerSheetAccent`` 를 그대로 쓴다(배지 위 5.29:1, 흰 카드 위 6.48:1).
    ///
    /// - Note: 정답을 파랑이 아니라 **녹색**으로 두는 편이 뜻에 맞고, 오답 창에서도
    ///   「고르신 답(분홍) ↔ 정답(녹색)」이 더 또렷하다. 한때 쓰던 하늘색 두 값은
    ///   2026-09-23 리팩토링에서 지웠다 (`Refactoring.md` 1단계).
    static let answerSheetBadge = Color(red: 0.82, green: 0.94, blue: 0.83)
    ...
    /// **실제로 화면에 보이던 색(흰 배경 위 opacity 0.8이 섞인 값, 약 RGB
    /// 0.93·0.49·0.62)을 기준으로 흰색 쪽으로 60% 더 섞었다** — opacity를 쓰지 않고
    /// 미리 섞은 값을 그대로 쓰는 건, 배지 배경은 뒤에 뭐가 있든 항상 같은 옅은
    /// 색으로 보여야 해서다.
    ///
    ...
    /// 흰 카드 위에서 고르신 답 낱말을 강조할 때 쓰는 글자색.
    ///
    /// `wrongHeader`보다 어둡게 잡았다 — 배지(옅은 분홍) 위에 올라가는 글자라,
    /// 밝은 색을 그대로 쓰면 대비가 부족해진다. `wrongHeader`를 60% 더 옅게 바꾼
    /// 뒤로 배지 위 대비는 오히려 더 넉넉해졌다.
