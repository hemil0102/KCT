...
///
/// 시그니처 색을 **꽉 채우는 것은 이 버튼뿐**입니다. 보기 선택(``ChoiceButton``)은
/// 상태이므로 채우지 않습니다. 이 구분이 "무엇을 눌러야 하나"를 한눈에 만들어 줍니다.
///
/// - Important: `isReady` 가 `false` 여도 **버튼은 눌립니다.** `disabled` 를 쓰지
///   않는 이유는, 눌렀는데 아무 일도 일어나지 않으면 어르신이 앱이 고장 났다고
///   생각하기 때문입니다. 눌리게 두고 `action` 안에서 무엇이 필요한지 알려 줍니다.
