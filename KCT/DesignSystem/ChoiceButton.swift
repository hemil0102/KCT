...
/// 탭하면 선택되는 큰 보기 버튼.
///
/// 선택된 모습은 **꽉 채운 시그니처 색을 쓰지 않습니다.** 채움은 "지금 눌러야 할
/// 주 행동"(``PrimaryActionButton``)에만 쓰고, 여기서는 옅은 배경 + 굵은 테두리 +
/// 체크로 표시합니다. 상태와 행동이 같은 모양이면 무엇을 눌러야 할지 헷갈립니다.
///
/// - Note: 색 하나로만 알리지 않습니다. 체크 아이콘과 테두리 굵기가 함께 바뀌어,
///   색 구분이 어려운 분도 선택 여부를 알 수 있습니다.
