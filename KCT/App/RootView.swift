...
/// 앱의 첫 화면.
///
/// 어르신이 앱을 열면 **아무것도 고르지 않고 곧바로 문제를 만나야** 하므로,
/// 홈 화면이나 모드 선택을 두지 않고 바로 ``QuizView`` 로 갑니다.
///
/// - Note: 실전 모드(``SessionMode/exam``)를 고르는 입구가 필요해지면 여기에
///   붙습니다. 그때도 기본 동작은 "열면 바로 연습" 을 유지하는 것이 좋습니다.
